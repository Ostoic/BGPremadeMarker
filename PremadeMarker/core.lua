...
function in_queue()
   for _, i in ipairs(iota(1, MAX_BATTLEFIELD_QUEUES)) do
      if select(1, {GetBattlefieldStatus(i)}) == 'queued' then
         return true
      end
   end
...
local function can_set_raid_marks(unit)
   return UnitIsPartyLeader(unit) or IsRaidOfficer(unit)
end

local function will_be_queued(event, arg)
   return  (event == 'UI_INFO_MESSAGE' and arg:find('Your group has joined the queue') ~= nil) or in_queue()
end

local function should_mark(event, arg)
   return in_party() and not UnitInBattleground('player') and can_set_raid_marks('player') and will_be_queued(event, arg)
end
...
local function should_convert_to_raid()
   return get_group_type() == 'party'
end
